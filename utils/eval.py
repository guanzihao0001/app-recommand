...
class ItemBasedCF_test():
    ...
    def evaluate(self, K, N, sim_matrix):
        self.n_sim_app = K
        self.n_rec_app = N
        ...
        for i, user in enumerate(self.trainSet):
            test_apps = self.testSet.get(user, {})
        ...
        if rec_count == 0 or rec_count_adj == 0 or test_count == 0 or self.app_count == 0:
            ...
            print('precisioin=%.4f\tprecision_adj=%.4f\nrecall=%.4f\ncoverage=%.4f'
                  % (precision, precision_adj, recall, coverage))
            return precision, precision_adj, recall, coverage
...
if __name__ == '__main__':
    ...
    precision, recall, coverage, precision_adj = 0, 0, 0, 0
    itemCF_test.get_dataset(data, pivot=0.75)
    #itemCF_test.sampleamount(200000)
    app_sim_matrix, app_aidlist = itemCF_test.calc_app_sim(10, 5)

    FILE_PATH = CONFIG_FILE['SAVE_PATH']
    with open(os.path.join(FILE_PATH, 'app_sim_matrix_test.json'), 'w')as file_obj:
        json.dump(app_sim_matrix, file_obj)
    print('Save successfully !')
    print(len(app_sim_matrix))


    precision, precision_adj, recall, coverage = itemCF_test.evaluate(10, N, app_sim_matrix)
    ...
    if precision == 0 and recall == 0:
        ...
        log.logger.info('model: {}, recommend app number: {}, precision: {}, recall: {}'.format(model, N, precision, recall))
